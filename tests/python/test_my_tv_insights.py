...
class InsightLibrary(MyTvInsightsMixin):
    def __init__(self, root: Path) -> None:
        self.my_tv_insights_path = root / "insights.json"
        self.config_lock = threading.RLock()
        self.my_tv_insights_lock = threading.Lock()
        self.my_tv_insights_closed = threading.Event()
        self.my_tv_insights_worker = None
    ...
    def save_my_tv_insights(self, value) -> None:
        self.state_database.save_my_tv_insights(value)

    def my_tv_viewing_store(self):
        return self.store
...
class MyTvInsightsTests(unittest.TestCase):
    ...
    def test_enrichment_uses_tmdb_title_details_without_provider_requests(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            library = InsightLibrary(Path(temporary))
            library.enrich_my_tv_insights()
            cache = library.my_tv_insights_cache()
            payload = library.my_tv_insights_payload(
                {key: value for key, value in library.store["titles"].items()
                 if library.my_tv_insights_watched(value)}, cache)
