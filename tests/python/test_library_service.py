...
class LibraryHttpTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # The production server keeps HTTP/1.1 connections alive and detaches
        # request workers so a slow browser cannot hold up a service shutdown.
        # The test needs the opposite: close each test request and wait for its
        # worker before removing the temporary on-disk library.  That avoids a
        # Pi 1 race where a completed response could still write owner.json.
        self.server.RequestHandlerClass.protocol_version = "HTTP/1.0"
