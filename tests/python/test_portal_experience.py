...
class PortalExperienceTests(unittest.TestCase):
    ...
    def test_remote_browser_player_has_native_controls_and_safe_default(self) -> None:
        ...
        self.assertIn("const episodeChannels = channels.filter", index)
        self.assertIn("const filmChannels = channels.filter", index)
        self.assertIn("grid.className = 'mabel-episode-channel-grid'", index)
        self.assertIn("card.className = 'watch-card watch-mabel-series-channel-card'", index)
        self.assertIn("? 'mabel-show-10-0.jpg'", index)
        self.assertIn("<h2>Film channels</h2>", index)
        self.assertIn("card.onclick = () => openChannel(channel, true)", index)
        self.assertNotIn("identity.className = 'mabel-show-identity'", index)
