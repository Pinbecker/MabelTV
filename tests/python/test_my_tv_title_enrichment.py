...
from mabeltv_backend.my_tv_metadata import MyTvMetadataMixin  # noqa: E402
...
class CatalogueFixture(MyTvMetadataMixin, ProviderTransportMixin):
    ...
    @staticmethod
    def my_tv_library() -> list[dict[str, Any]]:
        return []

    @staticmethod
    def my_tv_viewing_store() -> dict[str, Any]:
...
class MyTvTitleEnrichmentTests(unittest.TestCase):
    def test_discovery_includes_people_without_title_viewing_state(self) -> None:
        found = CatalogueFixture().my_tv_discovery("Jane")
    ...
    def test_provider_badge_setting_defaults_on_and_persists_off(self) -> None:
        ...
        self.assertNotEqual(library.settings().get("my_tv_provider_badges_enabled"), False)
        self.assertTrue(library.manage({"action": "set-my-tv-provider-badges",
                                        "enabled": False}))
        self.assertFalse(library.settings()["my_tv_provider_badges_enabled"])

    def test_movie_detail_normalises_and_caches_franchise_cast_and_director(self) -> None:
        library = CatalogueFixture()

        detail = library.my_tv_title_detail("movie", 12)
        repeated = library.my_tv_title_detail("movie", 12)
    ...
    def test_tv_detail_uses_exact_air_dates_creator_rating_and_cast(self) -> None:
        detail = CatalogueFixture().my_tv_title_detail("tv", 77)
    ...
    def test_person_detail_is_lazy_cacheable_and_keeps_ranked_credits(self) -> None:
        ...
        detail = library.my_tv_person_detail(1)
        repeated = library.my_tv_person_detail(1)
    ...
    def test_person_detail_uses_the_creative_department_for_directors(self) -> None:
        detail = CatalogueFixture().my_tv_person_detail(3)
    ...
    def test_person_credit_ranking_rejects_guest_and_archive_appearances(self) -> None:
        ...
        self.assertIsNotNone(CatalogueFixture.my_tv_person_credit_score(meaningful))
        self.assertIsNone(CatalogueFixture.my_tv_person_credit_score(guest))
        self.assertIsNone(CatalogueFixture.my_tv_person_credit_score(archive))
    ...
    def test_disabled_availability_skips_tmdb_providers_and_watchmode(self) -> None:
        ...
        detail = library.my_tv_title_detail("movie", 12)
        links = library.my_tv_streaming_links("movie", 12)
