...
class RemotePlaybackLibraryTests(unittest.TestCase):
    ...
    def test_lg_remote_discovers_app_ids_and_mabeltv_input(self) -> None:
        ...
        with mock.patch.object(library, "lg_tv_session", return_value=session), \
                mock.patch.object(library, "tv_identity",
                                  return_value=("Mabel", "Mabel TV")):
            result = library.lg_tv_switch_to_mabeltv()

        self.assertEqual(result["message"], "Switching to Mabel TV…")
    ...
    def test_remote_stream_requires_browser_format_and_resumes_my_tv_film(self) -> None:
        my_tv = self.fixture.media / ".my-tv"
        my_tv.mkdir(parents=True, exist_ok=True)
        film = my_tv / "Remote Film.mp4"
        film.write_bytes(b"remote-film")
        self.fixture.library.write_state("player", json.loads('{"standby": true}'))
        item = self.fixture.library.my_tv_library()[0]
        started = self.fixture.library.start_remote_stream({"kind": "my_tv", "file": "Remote Film.mp4"})
        ...
        self.assertEqual(self.fixture.library.my_tv_library()[0]["remote_position"], 42)
        (my_tv / "Remote Film.en.srt").write_text("1\n00:00:01,000 --> 00:00:02,000\nHello\n", encoding="utf-8")
        with_captions = self.fixture.library.start_remote_stream({"kind": "my_tv", "file": "Remote Film.mp4"})
        ...
        self.assertEqual(item["library_id"], self.fixture.library.my_tv_library()[0]["library_id"])

        from_beginning = self.fixture.library.start_remote_stream({
            "kind": "my_tv", "file": "Remote Film.mp4", "position": 0,
        })
    ...
    def test_film_favourites_are_shared_with_the_portal_without_refreshing_tv(self) -> None:
        ...
        my_tv = self.fixture.library.my_tv_root / "MyTv Film.mp4"
        my_tv.write_bytes(b"my_tv")
        self.fixture.library.refresh_tv = mock.Mock(return_value=True)

        self.fixture.library.set_favourite({
            "kind": "my_tv", "file": my_tv.name, "enabled": True,
        })
        self.fixture.library.set_favourite({
        ...
        self.assertTrue(rendered["my_tv_library"][0]["favourite"])
    ...
    def test_remote_stream_blocks_live_tv_unless_concurrent_mode_is_enabled(self) -> None:
        my_tv = self.fixture.media / ".my-tv"
        my_tv.mkdir(parents=True, exist_ok=True)
        (my_tv / "Film.mp4").write_bytes(b"film")
        ...
        with self.assertRaises(mabeltv_library.RemoteTvActiveError):
            self.fixture.library.start_remote_stream({"kind": "my_tv", "file": "Film.mp4"})
        self.fixture.library.manage({"action": "set-remote-simultaneous", "enabled": True})
        started = self.fixture.library.start_remote_stream({"kind": "my_tv", "file": "Film.mp4"})
    ...
    def test_remote_session_accepts_a_large_backward_seek(self) -> None:
        ...
        self.assertEqual(
            self.fixture.library.my_tv_library()[0]["remote_position"], 1200)
    ...
    def test_most_recent_my_tv_session_sets_next_shared_resume_position(self) -> None:
        my_tv = self.fixture.media / ".my-tv"
        my_tv.mkdir(parents=True, exist_ok=True)
        (my_tv / "Film.mp4").write_bytes(b"film")
        item = self.fixture.library.my_tv_library()[0]
        ...
        self.fixture.library.write_state("player", json.loads(json.dumps({
            "standby": False,
            "my_tv_positions": {item["library_id"]: 300},
            "my_tv_durations": {item["library_id"]: 7200},
            "my_tv_position_updated_utc_ms": {item["library_id"]: 300000},
        })))

        latest_tv = self.fixture.library.my_tv_library()[0]
        ...
        self.fixture.library.write_my_tv_media_states(states)
        latest_browser = self.fixture.library.my_tv_library()[0]
    ...
    def test_starting_over_suppresses_the_stale_tv_bookmark(self) -> None:
        ...
        item = self.fixture.library.my_tv_library()[0]
        self.fixture.library.write_state("player", json.loads(json.dumps({
            "standby": True, "my_tv_positions": {item["library_id"]: 420},
        })))
        started = self.fixture.library.start_remote_stream({"kind": "my_tv", "file": "Film.mp4"})
        token = started["stream_url"].split("stream=", 1)[1]
        self.fixture.library.remote_save_position({
        ...
        self.assertEqual(self.fixture.library.my_tv_library()[0]["remote_position"], 0)
        self.fixture.library.write_state("player", json.loads(json.dumps({
            "standby": True, "my_tv_positions": {item["library_id"]: 480},
        })))
        self.assertEqual(self.fixture.library.my_tv_library()[0]["remote_position"], 480)
        self.fixture.library.write_state("player", json.loads(json.dumps({
            "standby": True,
            "my_tv_positions": {item["library_id"]: 7050},
            "my_tv_durations": {item["library_id"]: 7200},
        })))
        finished = self.fixture.library.my_tv_library()[0]
    ...
    def test_explicit_continue_watching_removal_clears_browser_and_tv_bookmarks(self) -> None:
        ...
        self.fixture.library.write_state("player", json.loads(json.dumps({
            "standby": True,
            "my_tv_positions": {item["library_id"]: 600},
            "my_tv_durations": {item["library_id"]: 7200},
        })))
        ...
        cleared = self.fixture.library.my_tv_library()[0]
        self.assertEqual(cleared["remote_position"], 0)
        self.assertEqual(cleared["remote_last_watched"], 0)
        saved = self.fixture.library.my_tv_media_states()["Film.mp4"]
    ...
    def test_new_remote_stream_replaces_old_without_stale_token_clearing_it(self) -> None:
        my_tv = self.fixture.media / ".my-tv"
        my_tv.mkdir(parents=True, exist_ok=True)
        (my_tv / "First.mp4").write_bytes(b"first")
        (my_tv / "Second.mp4").write_bytes(b"second")
        self.fixture.library.write_state("player", json.loads('{"standby": true}'))
        first = self.fixture.library.start_remote_stream({"kind": "my_tv", "file": "First.mp4"})
        second = self.fixture.library.start_remote_stream({"kind": "my_tv", "file": "Second.mp4"})
