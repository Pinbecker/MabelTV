...
class SeriesAndChannelManagementTests(unittest.TestCase):
    ...
    def test_my_tv_series_groups_episodes_and_tracks_manual_watched_state(self) -> None:
        series_id = self.fixture.library.create_my_tv_series("Silicon Valley")
        season = self.fixture.library.my_tv_series_root / series_id / "Season 1"
        season.mkdir()
        ...
        self.fixture.library.set_favourite({
            "kind": "my-tv-series", "series": series_id, "enabled": True,
        })
        self.assertTrue(self.fixture.library.my_tv_series_library()[0]["favourite"])

        result = self.fixture.library.set_my_tv_episode_watched(
            series_id, "Season 1/Silicon.Valley.S01E06.720p.HDTV.x264.mkv", True)
        self.assertTrue(result["watched"])
        refreshed = self.fixture.library.my_tv_series_library()[0]
        ...
        bulk = self.fixture.library.set_my_tv_season_watched(series_id, 1, True)
        self.assertEqual(bulk["episodes_updated"], 1)
        self.assertTrue(bulk["episodes"][0]["watched"])
        bulk = self.fixture.library.set_my_tv_season_watched(series_id, 1, False)
    ...
    def test_my_tv_series_restart_clears_one_season_or_complete_show(self) -> None:
        series_id = self.fixture.library.create_my_tv_series("Silicon Valley")
        root = self.fixture.library.my_tv_series_root / series_id
        ...
        result = self.fixture.library.restart_my_tv_series_progress(
            series_id, "season", 1)
    ...
    def test_my_tv_series_direct_upload_publishes_into_selected_series_number(self) -> None:
        series_id = self.fixture.library.create_my_tv_series("Silicon Valley")
        ...
        created = self.fixture.library.my_tv_series_upload_create({
        ...
        self.assertEqual(state["kind"], "my-tv-series")
        self.assertEqual(state["series_id"], series_id)
        self.assertEqual(state["season"], 2)
        self.assertEqual(
            (self.fixture.library.my_tv_series_root / series_id / "Season 2" /
             "Silicon.Valley.S02E01.mp4").read_bytes(), b"episode")
        episode = self.fixture.library.my_tv_series_library()[0]["episodes"][0]
        self.assertEqual((episode["season"], episode["episode"]), (2, 1))
        self.fixture.library.refresh_tv.assert_not_called()

    def test_my_tv_series_portal_uses_scoped_series_and_episode_workflow(self) -> None:
        self.assertIn('id="myTvSeasonSheet"', PORTAL_SOURCE)
        self.assertIn('class="my-tv-series-seasons"', PORTAL_SOURCE)
        self.assertIn("function openMyTvSeasonSheet(series, season, returnTo = null, targetPath = '')", PORTAL_SOURCE)
        self.assertIn("openMyTvSeriesUpload(current, number)", PORTAL_SOURCE)
        self.assertIn("const season = Number(target?.season)", PORTAL_SOURCE)
        self.assertIn("Create Series ${nextSeries}", PORTAL_SOURCE)
        self.assertIn("action: 'create-my-tv-season'", PORTAL_SOURCE)
        self.assertIn("scope: 'season', season: number", PORTAL_SOURCE)
        self.assertIn('id="myTvSeriesSourceSheet"', PORTAL_SOURCE)
        self.assertIn('id="myTvSeriesSourceFiles"', PORTAL_SOURCE)
        self.assertIn('id="myTvSeriesSourceUsb"', PORTAL_SOURCE)
        self.assertIn("function openMyTvSeriesSourceSheet()", PORTAL_SOURCE)
        self.assertIn("function returnToMyTvSeriesUploadSheet()", PORTAL_SOURCE)
        self.assertNotIn('id="myTvSeriesUpload"', PORTAL_SOURCE)
        self.assertNotIn('id="myTvSeriesAddUsb"', PORTAL_SOURCE)
        self.assertNotIn('id="myTvSeriesUploadSeason"', PORTAL_SOURCE)
        self.assertNotIn('Use an existing number', PORTAL_SOURCE)
        self.assertNotIn('id="myTvSeasonBack"', PORTAL_SOURCE)
        self.assertIn('id="myTvSeriesRestartSheet"', PORTAL_SOURCE)
        self.assertIn('id="myTvSeasonRestart"', PORTAL_SOURCE)
        self.assertIn('id="myTvSeriesRestart"', PORTAL_SOURCE)
        self.assertIn("function myTvSeriesContinueEntries()", PORTAL_SOURCE)
        self.assertIn("[...resumableFilms, ...myTvSeriesContinueEntries()]",
                      PORTAL_SOURCE)
        ...
        self.assertNotIn("myTvSeriesContinueEntries", home_renderer.group(1))

    def test_my_tv_series_episode_season_and_show_cleanup_use_recycle_bin(self) -> None:
        series_id = self.fixture.library.create_my_tv_series("Silicon Valley")
        root = self.fixture.library.my_tv_series_root / series_id
        for season, episode in ((1, 1), (1, 2), (2, 1)):
            folder = root / f"Season {season}"
        ...
        remaining = self.fixture.library.my_tv_series_library()[0]
        ...
        restored = self.fixture.library.my_tv_series_library()[0]
        ...
        self.assertEqual(removed, 2)
        self.assertEqual(self.fixture.library.my_tv_series_library(), [])
    ...
    def test_pi_ready_my_tv_upload_is_kept_without_conversion(self) -> None:
        ...
        self.fixture.library.optimise_my_tv_for_playback = mock.Mock()
        self.fixture.library.refresh_tv = mock.Mock(return_value=True)
        ...
        self.assertEqual((self.fixture.library.my_tv_root / "Ready Film.mp4").read_bytes(),
                         b"ready")
        self.fixture.library.optimise_my_tv_for_playback.assert_not_called()

    def test_my_tv_folders_are_real_shared_collections_and_preserve_identity(self) -> None:
        self.fixture.library.complete_setup({
            "setup_code": "135790", "pin": "2468",
            "channels": mabeltv_library.DEFAULT_CHANNELS,
        })
        self.fixture.library.refresh_tv = mock.Mock(return_value=True)
        source = self.fixture.library.my_tv_root / "Fellowship.mkv"
        source.write_bytes(b"film")

        first = self.fixture.library.my_tv_library()[0]
        identity = first["library_id"]
        self.fixture.library.manage({
            "action": "create-my-tv-folder", "name": "The Lord of the Rings",
        })
        self.fixture.library.manage({
            "action": "move-my-tv", "file": first["path"],
            "folder": "The Lord of the Rings",
        })

        moved = self.fixture.library.my_tv_library()[0]
        ...
        self.fixture.library.manage({
            "action": "rename-my-tv-folder", "folder": "The Lord of the Rings",
            "name": "Middle-earth",
        })
        renamed = self.fixture.library.my_tv_library()[0]
        self.assertEqual(renamed["path"], "Middle-earth/Fellowship.mkv")
        self.assertEqual(renamed["library_id"], identity)
        self.assertEqual(self.fixture.library.library()["my_tv_folders"], ["Middle-earth"])

        with self.assertRaisesRegex(ValueError, "Move every film"):
            self.fixture.library.manage({
                "action": "delete-my-tv-folder", "folder": "Middle-earth",
            })
        self.fixture.library.manage({
            "action": "move-my-tv", "file": renamed["path"], "folder": "",
        })
        self.fixture.library.manage({
            "action": "delete-my-tv-folder", "folder": "Middle-earth",
        })
        self.assertEqual(self.fixture.library.my_tv_folders(), [])
