...
test('USB returns to the parent folder position and selection does not scroll', async ({ page }) => {
  ...
  await expect(page.locator('#usbFolderTitle')).toHaveText('Test drive')
  await settled(page)
  expect(await scrollY(page)).toBeCloseTo(before, 0)
