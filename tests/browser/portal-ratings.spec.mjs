...
test('Ratings is a four-wide watched-only queue and saves from the lightweight title card', async ({ page }, testInfo) => {
  ...
  await page.route(url => new URL(url).pathname === '/api/my-tv/title', async route => {
  ...
  await expect(page.locator('#view-my-tv-ratings')).toBeVisible()
  await expect(page.locator('#myTvRatingsGrid .my-tv-explore-card')).toHaveCount(5)
  await expect(page.locator('#myTvRatingsGrid [data-explore-action]')).toHaveCount(0)
  const firstRow = await page.locator('#myTvRatingsGrid .my-tv-explore-card').evaluateAll(cards =>
  ...
  await page.locator('#myTvRatingsGrid .my-tv-explore-card').first()
    .locator('.my-tv-explore-open-art').click()
  ...
  const rating = page.locator('#myTvTitlePersonalRating .my-tv-personal-rating')
  await expect(rating.locator('.my-tv-rating-star')).toHaveCount(10)
  const track = rating.locator('.my-tv-rating-track')
  const box = await track.boundingBox()
  await page.mouse.click(box.x + box.width * .66, box.y + box.height / 2)
  await expect(rating).toHaveAttribute('aria-valuenow', '7')
  await expect(page.locator('#myTvRatingsGrid .my-tv-explore-card')).toHaveCount(4)
  const geometry = await rating.evaluate(control => {
    const host = control.closest('.my-tv-personal-rating-host').getBoundingClientRect()
    const summary = control.closest('.watch-film-summary').getBoundingClientRect()
    const track = control.querySelector('.my-tv-rating-track').getBoundingClientRect()
    return {
      height: host.height,
  ...
  const clearRating = rating.locator('.my-tv-rating-label')
  ...
  const locked = page.locator('#myTvTitlePersonalRating .my-tv-personal-rating')
  await expect(locked).toHaveClass(/is-locked/)
  await expect(locked).toContainText('Mark watched to add a rating')
  await expect(locked.locator('.my-tv-rating-track')).toHaveCount(0)
  await page.locator('#myTvTitleIntents [data-viewing-action="watched"]').click()
  await expect(page.locator('#myTvTitlePersonalRating .my-tv-rating-star')).toHaveCount(10)
})
