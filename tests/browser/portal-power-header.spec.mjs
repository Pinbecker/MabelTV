...
test('the global power control replaces Home actions but yields to either remote', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#openHeaderAdultTv')).toBeVisible()
  ...
  await expect(page.locator('#openHeaderAdultTv')).toBeHidden()
...
test('the global Adult TV shortcut opens the Adult library directly', async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.startsWith('iphone-'), 'Phone header contract')
  await openPortal(page)
  await page.locator('#openHeaderAdultTv').click()
  await expect(page.locator('#view-watch')).toBeVisible()
  await expect(page.locator('#watchAdultTab')).toHaveAttribute('aria-selected', 'true')
