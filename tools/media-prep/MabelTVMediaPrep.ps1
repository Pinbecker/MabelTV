...
function New-DropZone([string]$Target, [int]$X, [string]$Heading, [string]$Description, [Drawing.Color]$Accent) {
    ...
    foreach ($control in @($panel, $head, $copy, $button, $hint)) { $control.Tag = $Target }
    $drop = { param($sender, $event) $event.Effect = [Windows.Forms.DragDropEffects]::Copy }
    $receive = { param($sender, $event) if ($event.Data.GetDataPresent([Windows.Forms.DataFormats]::FileDrop)) { Add-Files ([string]$sender.Tag) @($event.Data.GetData([Windows.Forms.DataFormats]::FileDrop)) } }
    ...
    $button.Add_Click({ param($sender, $event) $dialog = [Windows.Forms.OpenFileDialog]::new(); $dialog.Multiselect = $true; $dialog.Filter = 'Video files|*.mp4;*.m4v;*.mov;*.mkv;*.avi;*.mpg;*.mpeg;*.webm|All files|*.*'; if ($dialog.ShowDialog() -eq 'OK') { Add-Files ([string]$sender.Tag) $dialog.FileNames } })
    return $panel
}
