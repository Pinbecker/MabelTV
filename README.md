...
Mabel TV turns a Raspberry Pi 4 into a calm, child-friendly television made from a family’s own video library. It recreates the reassuring feel of channel surfing on a 1990s CRT while keeping playback local, offline, and under a grown-up’s control.
...
No account, cloud service, subscription, advertising, analytics, or internet playback is required. Mabel TV does not include programmes; owners supply media they are entitled to use.

## Start here

For an owner installing a release bundle:
...
The customer journey, including exact Windows/macOS/Linux copy commands, is covered in [Quick start](docs/quick-start.md). Source-tree installation, updates, rollback, uninstall, supported hardware, and release qualification are in [Raspberry Pi installation](docs/raspberry-pi-setup.md).
...
- Resumable 8 MiB uploads with durable offsets and conservative free-space reservation.
...
Mabel TV is licensed under GPL-3.0-or-later. Selling or redistributing it requires providing the corresponding source and licence notices for the exact distributed build. See [Third-party notices](THIRD_PARTY_NOTICES.md).

The Mabel TV software and its synthetic test assets are separate from the owner’s media library. Do not distribute films or programmes with a product unless you hold the necessary rights.
