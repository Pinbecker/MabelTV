...
| Cache Storage `mabeltv-artwork-protected-v1` | Same-origin My TV posters, backdrops, people and related TMDB artwork | Rebuildable and PIN-protected, bounded to 1,000 responses. |
...
The My TV provider-summary snapshot is shared by every My TV title grid.
...
Snapshots may contain private response data, including My TV metadata, so they
...
Family artwork can be served directly from its cache. My TV artwork is returned
...
- `/offline-media/<id>` and `/offline-subtitles/<id>.vtt` read the Downloads
  IndexedDB and enforce My TV access before returning bytes.
...
## Downloads and My TV protection
...
My TV manifests are marked `protected` and are also recognised by their source
...
- Test worker routing and storage upgrades with
  ...
  navigation, download survival and My TV locking with `pwa.spec.mjs` in both
