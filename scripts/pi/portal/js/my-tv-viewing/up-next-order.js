...
function upNextOrderKeys(root) {
  return [...root.querySelectorAll(':scope > .my-tv-viewing-row')]
...
function animateUpNextNeighbours(root, before) {
  for (const row of root.querySelectorAll(':scope > .my-tv-viewing-row')) {
...
async function saveUpNextOrder(keys, previous) {
  ...
  myTvHomeLoadedAt = 0
  try {
    await api('/api/my-tv/viewing/reorder', {
...
function bindUpNextReorder(root) {
  const rows = [...root.querySelectorAll(':scope > .my-tv-viewing-row')]
  if (rows.length < 2) return
  root.classList.add('is-reorderable')
  rows.forEach(row => {
    const surface = row.querySelector('.my-tv-viewing-card-open')
    ...
    const reorderAtPointer = () => {
      const candidates = [...root.querySelectorAll(':scope > .my-tv-viewing-row')]
    ...
    const activate = () => {
      ...
      floating.classList.add('my-tv-viewing-drag-proxy')
