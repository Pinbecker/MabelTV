...
let myTvDiscoveryTimer = null
let myTvDiscoveryRevision = 0
let myTvViewingData = { items: [] }
let myTvViewingLoaded = false
let myTvViewingTab = 'watchlist'
let myTvViewingFilter = 'all'
let myTvViewingSearch = ''
let myTvViewingSort = 'recent'
let myTvViewingDataRevision = 0
let selectedMyTvTitle = null
let pendingNetflixLaunch = null
let myTvTitleOpenRevision = 0
let myTvSeasonOpenRevision = 0
let myTvViewingMutationRevision = 0
const myTvViewingTitleMutations = new Map()
let myTvSearchViewportBaseline = Math.max(
  window.innerHeight, window.visualViewport?.height || 0)
let myTvSearchKeyboardWasOpen = false


function myTvViewingRecord(title = {}) {
...
function cacheMyTvViewingRecord(viewing = {}) {
  if (!viewing.key) return
  const items = myTvViewingData.items || (myTvViewingData.items = [])
...
function optimisticMyTvViewingRecord(title, action, extra = {}) {
  const current = myTvViewingRecord(title)
...
function myTvArtworkStatusKind(title = {}, detail = title) {
  const state = myTvViewingRecord(title)
  const status = myTvTitleViewingStatus(state, {
...
function appendMyTvArtworkStatus(root, title = {}, detail = title) {
  root.classList.add('my-tv-artwork-status-host')
  root._myTvArtworkTitle = title
  root._myTvArtworkDetail = detail
  root.querySelector(':scope > .my-tv-artwork-status')?.remove()
  const kind = myTvArtworkStatusKind(title, detail)
...
function refreshMyTvArtworkStatuses() {
  $$('.my-tv-artwork-status-host').forEach(root => appendMyTvArtworkStatus(
    root, root._myTvArtworkTitle || {}, root._myTvArtworkDetail || {}))
  if (typeof refreshMyTvExploreCards === 'function') refreshMyTvExploreCards()
}

function appendMyTvLocalArtworkStatus(root, mediaType, value = {}) {
...
function myTvProviderBrandFor(record, idField, brandIds) {
  const identifier = Number(record?.[idField] || 0)
  return (identifier && myTvProviderBrands.find(brand => brand[brandIds].includes(identifier)))
    || myTvProviderBrands.find(brand => brand.match.test(String(record?.name || ''))
      && !(brand.id === 'prime-video' && /\bvia\s+amazon\s+prime\b/i.test(String(record?.name || ''))))
}

function myTvAvailabilityEnabled() {
  return library?.my_tv_settings?.watchmode_availability_enabled !== false
}

function myTvProviderPlatform(client = navigator) {
...
function myTvProviderDestination(brand, source, title, platform = myTvProviderPlatform()) {
...
function myTvProviderAssetUrl(brand) {
  return /^https:\/\//i.test(brand.asset)
    ? brand.asset : `/portal/assets/providers/${brand.asset}`
}

function myTvSourceDestination(source, platform = myTvProviderPlatform()) {
  const fields = platform === 'ios'
...
function myTvPurchaseProviderKey(name) {
...
function myTvPurchaseProviderAsset(group, detail) {
  const source = group.sources[0]
  const brand = myTvProviderBrandFor(source, 'source_id', 'watchmodeIds')
  if (brand) return myTvProviderAssetUrl(brand)
  const provider = (detail.providers || []).find(value =>
    myTvPurchaseProviderKey(value.name) === myTvPurchaseProviderKey(group.name)
    && value.logo_path)
  return provider?.logo_path ? myTvPosterUrl(provider.logo_path, 'w92') : ''
}

function renderMyTvPurchaseOffersInto(section, root, detail, result) {
  if (!section || !root) return
  root.replaceChildren()
  root.hidden = true
  section.classList.remove('is-expanded')
  const toggle = section.querySelector('.my-tv-rent-toggle')
  const summary = section.querySelector('.my-tv-rent-summary')
  if (toggle) {
  ...
  const add = source => {
    ...
    const marker = myTvPurchaseProviderKey(name)
  ...
  if (offers.length) {
    ...
    providers.className = 'my-tv-purchase-providers'
    offers.forEach(group => {
      const destinationSource = group.sources.find(source => myTvSourceDestination(source))
      const brand = myTvProviderBrandFor(group.provider, 'provider_id', 'tmdbIds')
        || myTvProviderBrandFor(destinationSource, 'source_id', 'watchmodeIds')
      const destination = myTvSourceDestination(destinationSource)
        || (brand ? myTvProviderDestination(brand, null, detail.title) : '')
      const offer = document.createElement(destination ? 'button' : 'div')
      if (destination) offer.type = 'button'
      offer.className = 'my-tv-purchase-provider'
      offer.dataset.provider = myTvPurchaseProviderKey(group.name)
      const logo = document.createElement('span')
      logo.className = 'my-tv-purchase-logo'
      const asset = myTvPurchaseProviderAsset(group, detail)
      if (asset) {
...
function openNetflixLaunchChoice(detail, provider, brand, source) {
  const deviceDestination = myTvProviderDestination(brand, source, detail.title)
  const tvDestination = myTvProviderDestination(brand, source, detail.title, 'web')
  ...
  $('#myTvNetflixLaunchTitle').textContent = `Play ${detail.title} where?`
  $('#myTvNetflixLaunchCopy').textContent = 'Open Netflix here, or send this title to the connected TV.'
  $('#myTvNetflixLaunchStatus').textContent = tvAvailable ? '' : 'Netflix did not provide a direct TV destination for this title.'
  $('#myTvNetflixLaunchTv').disabled = !tvAvailable
  portalSheets.open($('#myTvNetflixLaunchSheet'))
}

function closeNetflixLaunchChoice() {
  pendingNetflixLaunch = null
  portalSheets.dismiss($('#myTvNetflixLaunchSheet'))
}
...
function launchNetflixOnDevice() {
  ...
  void updateMyTvViewing(pending.detail, 'launched', { provider: pending.provider.name }).catch(() => {})
...
async function launchNetflixOnTv() {
  ...
  const buttons = [$('#myTvNetflixLaunchDevice'), $('#myTvNetflixLaunchTv')]
  buttons.forEach(button => { button.disabled = true })
  $('#myTvNetflixLaunchStatus').textContent = 'Opening Netflix on the connected TV…'
  try {
    const result = await api('/api/my-tv/netflix/play-tv', {
    ...
    $('#myTvNetflixLaunchStatus').textContent = error.message || 'Netflix could not open on the TV.'
  } finally {
    buttons.forEach(button => { button.disabled = !pending.tvAvailable })
  }
}

function myTvTitlePayload(title, action, extra = {}) {
  return {
...
async function updateMyTvViewing(title, action, extra = {}, onChange = null) {
  const key = title.key || `${title.media_type}:${Number(title.tmdb_id)}`
  myTvViewingMutationRevision += 1
  const titleMutation = Number(myTvViewingTitleMutations.get(key) || 0) + 1
  myTvViewingTitleMutations.set(key, titleMutation)
  const items = myTvViewingData.items || (myTvViewingData.items = [])
  ...
  const apply = viewing => {
    ...
    cacheMyTvViewingRecord(viewing)
    refreshMyTvArtworkStatuses()
    onChange?.(viewing)
  }
  apply(optimisticMyTvViewingRecord(title, action, extra))
  try {
    const result = await api('/api/my-tv/viewing', {
      method: 'POST', body: JSON.stringify(myTvTitlePayload(title, action, extra)),
    })
    if (myTvViewingTitleMutations.get(key) === titleMutation) {
      apply({ ...result.viewing, key: result.key })
    }
    void persistMyTvViewingCache()
...
function myTvDiscoveryCard(title) {
  ...
  if (artwork) {
    ...
    image.loading = 'lazy'
    image.decoding = 'async'
    image.src = myTvPosterUrl(artwork, person ? 'w342' : 'w342')
    image.alt = ''
    art.append(image)
  ...
  card.onclick = () => {
    ...
    if (person) openMyTvPerson(title, '')
    else openMyTvTitle(title)
  }
  return card
}

function syncMyTvSearchMode() {
  const input = $('#watchSearch')
  const view = $('#view-my-tv-home')
  if (!input || !view) return
  const focused = document.activeElement === input
  const query = input.value.trim()
  const active = focused || Boolean(query)
  view.classList.toggle('my-tv-search-mode', active)
  if (active && query.length < 2) {
    $('#myTvDiscoverySection').classList.add('hidden')
    $('#myTvDiscoveryCount').textContent = ''
  } else if (!active) {
    $('#myTvDiscoverySection').classList.add('hidden')
  }
}

function renderMyTvDiscoveryLoading(root) {
...
function syncMyTvSearchKeyboard() {
  const viewport = window.visualViewport
  if (!viewport) return
  myTvSearchViewportBaseline = Math.max(myTvSearchViewportBaseline, viewport.height)
  const keyboardOpen = myTvSearchViewportBaseline - viewport.height > 120
  if (keyboardOpen) {
    myTvSearchKeyboardWasOpen = true
    return
  }
  if (!myTvSearchKeyboardWasOpen) return
...
async function searchMyTvDiscovery(query) {
  const revision = ++myTvDiscoveryRevision
  const section = $('#myTvDiscoverySection')
  const root = $('#myTvDiscoveryGrid')
  ...
  renderMyTvDiscoveryLoading(root)
  try {
    const result = await api(`/api/my-tv/discovery?q=${encodeURIComponent(query.trim())}`)
    if (revision !== myTvDiscoveryRevision) return
    root.replaceChildren()
    result.results.forEach(title => root.append(myTvDiscoveryCard(title)))
    $('#myTvDiscoveryCount').textContent = `${result.results.length} result${result.results.length === 1 ? '' : 's'}`
...
function scheduleMyTvDiscovery() {
  clearTimeout(myTvDiscoveryTimer)
  const query = $('#watchSearch')?.value || ''
  syncMyTvSearchMode()
  if (query.trim().length < 2) searchMyTvDiscovery('')
  else myTvDiscoveryTimer = setTimeout(() => searchMyTvDiscovery(query), 320)
}

function localMyTvAction(detail) {
  if (!detail.local) return null
  const titleSheet = $('#myTvTitleSheet')
  const titleReturnTo = portalSheets.returnTo(titleSheet)
  const restoreTitle = () => restoreMyTvTitleSheet(detail, titleReturnTo)
  if (detail.local.kind === 'film') {
    const film = (library?.my_tv_library || []).find(item => item.path === detail.local.path)
    return film ? () => {
...
function renderMyTvProviderLinksInto(root, detail, result, options = {}) {
  root.replaceChildren()
  const localAction = Object.prototype.hasOwnProperty.call(options, 'localAction')
    ? options.localAction : localMyTvAction(detail)
  ...
  ;(detail.providers || []).forEach(provider => {
    ...
    const brand = myTvProviderBrandFor(provider, 'provider_id', 'tmdbIds')
  ...
  sourcesByBrand.forEach((source, brandId) => {
    ...
    const brand = myTvProviderBrands.find(value => value.id === brandId)
    if (brand) includedByBrand.set(brandId, { brand, provider: source })
  })
  const streaming = document.createElement('div')
  streaming.className = 'my-tv-provider-logos'
  if (detail.on_mabeltv) {
    const local = document.createElement(localAction ? 'button' : 'span')
    if (localAction) local.type = 'button'
    local.className = `my-tv-provider-logo provider-mabeltv${localAction ? '' : ' is-availability'}`
    local.setAttribute('aria-label', localAction ? `Open on ${tvName()}` : `Available on ${tvName()}`)
    local.title = localAction ? `Open on ${tvName()}` : `Available on ${tvName()}`
    const image = document.createElement('img')
    image.loading = 'lazy'
    image.decoding = 'async'
    image.src = '/apple-touch-icon.png'; image.alt = tvName()
    local.append(image)
    if (localAction) local.onclick = () => { beforeLaunch(); localAction() }
    streaming.append(local)
  }
  myTvProviderBrands.forEach(brand => {
    const included = includedByBrand.get(brand.id)
    if (!included) return
    const { provider } = included
    const button = document.createElement('button')
    button.type = 'button'; button.className = `my-tv-provider-logo provider-${brand.id}`
    button.setAttribute('aria-label', `Open ${brand.label}`)
    button.title = brand.label
    const image = document.createElement('img')
    image.loading = 'lazy'
    image.decoding = 'async'
    image.src = myTvProviderAssetUrl(brand)
    ...
    button.onclick = () => {
      ...
      const destination = myTvProviderDestination(brand, source, detail.title)
      ...
      void updateMyTvViewing(detail, 'launched', { provider: provider.name }).catch(() => {})
  ...
  renderMyTvPurchaseOffersInto(options.purchaseSection, options.purchaseRoot,
    detail, result)
}

function renderMyTvProviderLinks(detail, result) {
  renderMyTvProviderLinksInto($('#myTvProviderList'), detail, result, {
    localAction: null,
    purchaseSection: $('#myTvTitleRentBuy'), purchaseRoot: $('#myTvTitleRentBuyList'),
  })
}

function renderMyTvAvailabilityDisabled(root, detail, options = {}) {
  renderMyTvProviderLinksInto(root, { ...detail, providers: [] }, { sources: [] }, options)
...
async function loadMyTvProviders(detail, refresh = false, revision = myTvTitleOpenRevision) {
  const root = $('#myTvProviderList')
  if (!myTvAvailabilityEnabled()) {
    renderMyTvAvailabilityDisabled(root, detail, {
      localAction: null,
      purchaseSection: $('#myTvTitleRentBuy'), purchaseRoot: $('#myTvTitleRentBuyList'),
    })
    return
  ...
  if (previousResult) renderMyTvProviderLinks(detail, previousResult)
  else if ((detail.providers || []).length) {
    renderMyTvProviderLinks(detail, { sources: [] })
  } else {
  ...
  try {
    const result = await api(`/api/my-tv/providers?media_type=${detail.media_type}&tmdb_id=${detail.tmdb_id}${refresh ? '&refresh=1' : ''}`)
    if (revision !== myTvTitleOpenRevision || selectedMyTvTitle?.key !== detail.key) return
    detail.provider_result = result
    rememberMyTvTitleMetadata(detail)
    if (JSON.stringify(previousResult || null) !== JSON.stringify(result)) {
      renderMyTvProviderLinks(detail, result)
    }
    void writePortalDataCache(
      `my-tv-title-v1:${detail.media_type}:${Number(detail.tmdb_id)}`, detail)
  } catch (error) {
    if (revision !== myTvTitleOpenRevision || selectedMyTvTitle?.key !== detail.key) return
...
function myTvTitleViewingStatus(state = {}, detail = {}) {
...
function myTvViewingActionButtons(root) {
...
function syncMyTvTitleButtons(detail, root = $('#myTvTitleIntents')) {
  ...
  const status = myTvTitleViewingStatus(state, detail)
