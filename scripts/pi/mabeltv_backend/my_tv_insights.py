"""Personal catalogue insights built from My TV viewing records and TMDB."""
...
class MyTvInsightsMixin:
    ...
    def my_tv_insights_cache(self) -> dict[str, Any]:
        value = self.read_state("my_tv_insights")
    ...
    def write_my_tv_insights_cache(self, value: dict[str, Any]) -> None:
        value["updated"] = time.time()
        self.save_my_tv_insights(value)

    @staticmethod
    def my_tv_insights_metadata(value: dict[str, Any], media_type: str) -> dict[str, Any]:
    ...
    def my_tv_insights_missing(self, watched: dict[str, dict[str, Any]],
    ...
    def start_my_tv_insights_enrichment(self) -> None:
        if not self.tmdb_key() or self.my_tv_insights_closed.is_set():
            return
        with self.my_tv_insights_lock:
            if self.my_tv_insights_worker and self.my_tv_insights_worker.is_alive():
                return
            self.my_tv_insights_worker = threading.Thread(
                target=self.enrich_my_tv_insights,
                name="mabeltv-my-tv-insights", daemon=True)
            self.my_tv_insights_worker.start()

    def enrich_my_tv_insights(self) -> None:
        with self.config_lock:
            store = self.my_tv_viewing_store()
            watched = {key: dict(item) for key, item in store["titles"].items()
                       if isinstance(item, dict) and self.my_tv_insights_watched(item)}
            cache = self.my_tv_insights_cache()
            missing = self.my_tv_insights_missing(watched, cache)
        changed = 0
        for key, item in missing:
            if self.my_tv_insights_closed.is_set():
                break
            ...
            try:
                ...
                metadata = self.my_tv_insights_metadata(value, media_type)
                with self.config_lock:
            ...
            if changed % 10 == 0:
                with self.config_lock:
                    self.write_my_tv_insights_cache(cache)
            self.my_tv_insights_closed.wait(0.14)
        if changed:
            with self.config_lock:
                self.write_my_tv_insights_cache(cache)

    @staticmethod
    def my_tv_insights_people(metadata: dict[str, dict[str, Any]], field: str,
    ...
    @staticmethod
    def my_tv_insights_breakdown(counter: Counter[str], total: int,
                                 limit: int = 12) -> list[dict[str, Any]]:
        return [{"label": label, "count": count,
                 "share": round(count / total, 4) if total else 0}
                for label, count in counter.most_common(limit)]

    def my_tv_insights_payload(self, watched: dict[str, dict[str, Any]],
        ...
        return {
            ...
            "media_types": self.my_tv_insights_breakdown(media_types, total),
            "genres": self.my_tv_insights_breakdown(genres, total),
            "decades": [{"label": label, "count": decades[label]}
                        for label in sorted(decades)],
            "languages": self.my_tv_insights_breakdown(languages, enriched, 8),
            "countries": self.my_tv_insights_breakdown(countries, enriched, 8),
            "actors": self.my_tv_insights_people(metadata, "cast", ratings),
            "creative": self.my_tv_insights_people(metadata, "creative", ratings),
            ...
            "basis": "My TV watched history and personal ratings",
        }
    ...
    def my_tv_insights(self) -> dict[str, Any]:
        with self.config_lock:
            ...
            watched = {key: deepcopy(item) for key, item in store["titles"].items()
                       if isinstance(item, dict) and self.my_tv_insights_watched(item)}
            cache = self.my_tv_insights_cache()
            payload = self.my_tv_insights_payload(watched, cache)
        if not payload["enrichment"]["complete"]:
            self.start_my_tv_insights_enrichment()
        return payload
