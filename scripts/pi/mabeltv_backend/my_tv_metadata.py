"""My TV catalogue metadata and viewing behaviour for the local library service."""
...
class MyTvMetadataMixin:
    ...
    @staticmethod
    def my_tv_title_key(media_type: str, tmdb_id: Any) -> str:
    ...
    def my_tv_viewing_store(self) -> dict[str, Any]:
        value = self.read_state("my_tv_viewing")
        ...
        cutoff = time.time() - MY_TV_PROVIDER_MAX_CACHE_SECONDS
        value["availability"] = {
    ...
    def reset_my_tv_series_viewing_progress(
    ...
    @staticmethod
    def my_tv_title_summary(value: dict[str, Any], media_type: str) -> dict[str, Any]:
    ...
    @staticmethod
    def my_tv_next_episode_after_progress(
    ...
    def my_tv_local_title_index(self) -> dict[str, dict[str, Any]]:
        """Map My TV files to canonical titles, excluding family channels."""
        index: dict[str, dict[str, Any]] = {}
        for film in self.my_tv_library():
            metadata = film.get("metadata", {})
            try:
                key = self.my_tv_title_key("movie", metadata.get("tmdb_id"))
            except ValueError:
                continue
            index[key] = {
        ...
        for series in self.my_tv_series_library():
            ...
            try:
                ...
                continue
            next_episode = self.my_tv_next_episode_after_progress(episodes)
            index[key] = {
    ...
    def my_tv_discovery(self, query: str) -> dict[str, Any]:
    ...
    def my_tv_title_provider_groups(self, media_type: str, tmdb_id: Any) \
            -> dict[str, Any]:
    ...
    def my_tv_title_detail(self, media_type: str, tmdb_id: Any) -> dict[str, Any]:
        ...
        summary = self.my_tv_title_summary(value, media_type)
        if media_type == "movie":
        ...
        if media_type == "movie" and isinstance(collection_summary, dict) and \
            ...
            collection_value = self.my_tv_cached_tmdb_request(
                f"collection/{collection_id}", {"language": "en-GB"})
            parts = []
            for item in collection_value.get("parts", []) \
                    if isinstance(collection_value, dict) else []:
                if not isinstance(item, dict) or item.get("my_tv") is True:
                    continue
                part = self.my_tv_title_summary(item, "movie")
                if not part["tmdb_id"] or not part["title"]:
                    continue
                part["key"] = self.my_tv_title_key("movie", part["tmdb_id"])
                parts.append(part)
        ...
        local_titles = self.my_tv_local_title_index()
        if collection:
        ...
        if isinstance(local_title, dict) and local_title.get("kind") == "series":
            local_series = next(
                (series for series in self.my_tv_series_library()
        ...
        candidate = self.my_tv_next_episode_after_progress(available)
        next_episode = None
        if candidate:
            next_episode = {
    ...
    def my_tv_person_detail(self, tmdb_id: Any) -> dict[str, Any]:
        try:
        ...
        value = self.my_tv_cached_tmdb_request(f"person/{person_id}", {
        ...
        for item in display_credits:
            ...
            if media_type not in {"movie", "tv"} or not character or re.search(
                    ...
                    character.casefold()):
                continue
            summary = self.my_tv_title_summary(item, media_type)
            if not summary["tmdb_id"] or not summary["title"]:
                continue
            key = self.my_tv_title_key(media_type, summary["tmdb_id"])
        ...
        ranked = sorted(
            (item for item in display_credits
             if self.my_tv_person_credit_score(item) is not None),
            key=lambda item: self.my_tv_person_credit_score(item) or (),
            reverse=True,
        )
        ...
        for item in ranked:
            media_type = str(item.get("media_type") or "")
            if media_type not in {"movie", "tv"} or item.get("my_tv") is True:
                continue
            summary = self.my_tv_title_summary(item, media_type)
            if not summary["tmdb_id"] or not summary["title"]:
                continue
    ...
    @staticmethod
    def my_tv_person_credit_score(item: dict[str, Any]) -> tuple[float, int, float] | None:
    ...
    def my_tv_title_season(self, tmdb_id: Any, season_number: Any) -> dict[str, Any]:
        key = self.my_tv_title_key("tv", tmdb_id)
        try:
        ...
        with self.config_lock:
            ...
            if not isinstance(episode_states, dict):
                episode_states = {}
        local_title = self.my_tv_local_title_index().get(key, {})
        local_episode_states: dict[str, dict[str, Any]] = {}
        if isinstance(local_title, dict) and local_title.get("kind") == "series":
            local_series = next(
                (series for series in self.my_tv_series_library()
    ...
    def my_tv_streaming_links(self, media_type: str, tmdb_id: Any,
        ...
        with self.config_lock:
            ...
            if not refresh and isinstance(cached, dict) and \
                    ...
                    now - float(cached.get("checked", 0) or 0) < MY_TV_PROVIDER_CACHE_SECONDS:
                return dict(cached)
        ...
        result = {"key": key, "region": "GB", "checked": now, "link_schema": 3,
                  ...
                  "provider": "Watchmode"}
        with self.config_lock:
            self.save_my_tv_availability(key, result)
        return result

    def my_tv_viewing_update(self, payload: dict[str, Any]) -> dict[str, Any]:
        key = self.my_tv_title_key(payload.get("media_type"), payload.get("tmdb_id"))
        ...
        with self.config_lock:
            ...
            self.save_my_tv_titles(changed_titles)
            ...
            if isinstance(local_series, dict):
                if action == "episode_watched":
                    ...
                    if isinstance(target, dict) and target.get("path"):
                        self.set_my_tv_episode_watched(
