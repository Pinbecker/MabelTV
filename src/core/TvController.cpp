...
bool TvController::initialize(const QString &databasePath,
                              ...
                              std::function<qint64()> uptimeClock)
{
    ...
    if (mediaInspector) {
        library = ChannelLibrary::load(m_databasePath, m_mediaRoot,
                                       std::move(mediaInspector));
    } else {
        const QString cachePath = QDir(QFileInfo(m_databasePath).absolutePath())
                                      .filePath(QStringLiteral("media-index.json"));
        MediaIndex mediaIndex(cachePath);
        library = ChannelLibrary::load(m_databasePath, m_mediaRoot, [&mediaIndex](const QString &path) {
